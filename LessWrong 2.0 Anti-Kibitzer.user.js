...
function censorship_sweep() {
    ...
    // Evan: Get notification elements, except ones with loading spinner which was rending an error message and making those items unreadable
    update_censor_text(".NotificationsItem-notificationLabel:not(:has(.Loading-spinner))", notification_censor_bar, false);
    ...
    update_censor_text(".AgreementVoteAxis-agreementScore", vote_censor_bar, true);
    update_censor_text(".SingleLineComment-karma", vote_censor_bar, true);
    update_censor_text(".UsersProfile-meta", vote_censor_bar, true);
